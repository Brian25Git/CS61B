...
class Model {
    ...
    boolean placeable(Piece piece, int row, int col) {
        ...
        for (int i = 0; i < piece.height(); i++) {
            for (int j = 0; j < piece.width(); j++) {
                boolean pieceStructure = piece.get(i, j);
                boolean boardStructure = get(i + row, j + col);
                if (pieceStructure & boardStructure) {
                    return false;
                }
            }
        }
    ...
    void clearFilledLines() {
        ...
        _score += nrows * _width + ncols * _height - nrows * ncols;
    }
    ...
    private int scoreClearedLines(int nrows, int ncols) {
        int increase = nrows * _width + ncols * _height;
        if (nrows > 0 || ncols > 0) {
            _streakLength++;
        } else {
            _streakLength = 0;
        }
        return increase * _streakLength;
    }
